...
function renderTaskPage(taskId, url, response) {
    ...
    return runLogs.forTask(task).allRuns().then(function(runs) {
...
function renderTaskRunPage(taskId, runNumber, url, response) {
    var task = taskRepository.findTaskById(parseInt(taskId, 10));
    return runLogs.forTask(task).forRunNumber(runNumber).fetch()
        .then(function(run) {
...
function renderRunTaskPage(taskId, url, response) {
    var task = taskRepository.findTaskById(parseInt(taskId, 10));
    return task.startRun().then(function(runNumber) {
        var context = {currentUrl: url, task: task};
