...
RunLogsForRun.prototype.logResult = function(runResult) {
    ...
    return fs.mkdirp(path.dirname(resultFile)).then(function() {
...
RunLogsForRun.prototype.logOutput = function(output) {
    var self = this;
    var fs = this._fs;
    var outputFile = this._outputFile();
    return fs.mkdirp(path.dirname(outputFile)).then(function() {
        return fs.appendFile(outputFile, output);
    }).then(function() {
        return self;
    });
};
...
RunLogsForRun.prototype._outputFile = function() {
    var root = runsRootForTask(this._task);
    return path.join(root, this._runNumber.toString(), "output");
};
