...
function readConfig(templates, config) {
    var template = templates[config.action];
...
Task.prototype._setUpNextRun = function() {
    ...
    return this._setUpDirectories().then(function() {
        ...
        return q.ncall(mkdirp, null, runRoot).then(function() {
            return createRun(task, task._template, runNumber);
        });
    });
};

function createRun(task, template, runNumber) {
    function execute() {
        var startDateTime = new Date();
        return template(task).then(function(result) {
