...
function checkout(task) {
    var uri = task.config.action.uri;
...
function execTask(task) {
    ...
    var command = path.resolve(path.join(taskDirectory, task.config.action.command));
    ...
    function createEnv() {
        ...
        var configEnv = task.config.action.env;
...
function exec(task) {
    return executeCommand(task.config.action.command, {
        cwd: task.workspace
    });
}
